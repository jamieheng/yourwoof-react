    ...
    useEffect(() => {
      ...
      const handleSubmitFunc = async (e) => {
        ...
        try {
          ...
          const user = loggedemail !== "" && loggedpassword !== "" && authData.find((val) => val.adminEmail.toLowerCase() === loggedemail.toLowerCase() && val.adminPassword === loggedpassword);
          ...
          if (!user || !checkRole) {
            ...
            const response = await axios.get(`http://localhost:3001/admin/${user.id}`);
            if (response.status === 200) {
      ...
      <div className="min-h-screen w-screen bg-white flex flex-col lg:flex-row overflow-hidden  ">
        ...
        <div className="table-container w-full h-screen  lg:w-1/2 p-2 flex flex-col justify-start  lg:justify-center items-center bg-purple">
            ...
            <div className="form-wrapper w-full ">
                ...
                <form onSubmit={handleSubmitFunc} className='flex flex-col justify-center items-center w-full font-raleway bg-gradient-to-r from-white rounded-lg py-16'>
